...
' Optimize                       = No
' Stacksize                      = 1000
' Info_Last_Save                 = FERMIUM  Fermium\coldatoms
'<Header End>
#include ADwpad.inc
#include Adwpda.inc
#include Adwpdio.inc
...
'Dec 08/2022
' Added 3rd digital out

'Sept 24/2020
' DDS channel asignment understood. Digital channel numbers
' in sequencer GUI are ADwin code channel number +1
' Commented out DDS2 code and channel assignment, and ioreset for DDS1
' added some clarifying comments

'Sept 04/2020
' Added 8 analog channels for new analog card, assumed new card will be module 6
' Confused by DDS channel numbering scheme, don't understand DDS channel assignment
' Perhaps digital module 1 is the transistor output card
  
'Description:
'expecting 3 arrays.
dim DATA_1[5000000] as long  'A list which specifies how many channels will be updated per event
dim DATA_2[5000000] as long  'A list of channels to be updated
dim data_3[5000000] as float 'A list of values for the channels
dim data_4[5000000] as long  'A list of digital values to be updated
'DATA_2 and data_3 have a 1:1 correspondence.  
'Stepping through DATA_2 and data_3 is controlled by DATA_1
'Computer send an integer to Par_1.  This tells us how many
'events to write. 

'data_4 is the enactment of the reset to zero function in the sequencer code
dim data_5[100] as long     ' a list of which channels are reset to zero on completion


'Channels 1-48 refer to Analog output lines
'Channel 101,102 refers to Digital output lines (32 bit words each)
' (at one point, 101 and 102 were each 16 bit, perhaps when using a T9 processor)
...
'converts voltage to bit number
'value is from -10 to 10
'd1vides by 20 to make 0 to 1
'multiplies by 2^16 -1
...
INIT:
  delay=PAR_2 'set by sequencer: 40000=>1ms
  GlobalDelay=delay
  ...
  'initialize modules, 65535 -> all 16 channels set to 1 (output)
  DigProg1(1,65535)
  DigProg2(1,65535)
  DigProg1(2,65535)
  DigProg2(2,65535)
  ...
  ' digital: 1 is transistor output, not used, 2 and 3 are digital outs
  SYNCENABLE(1,dio,1)
  SYNCENABLE(2,dio,1)
  SYNCENABLE(3,dio,1)
  SYNCENABLE(4,dio,1) 'for third digital out card

  ' analog: 6 analog cards
  SYNCENABLE(1,da,1)
  SYNCENABLE(2,da,1)
  SYNCENABLE(3,da,1)
  SYNCENABLE(4,da,1)
  ...
  numbertoskip=0
  counts=1
  maxcount=Par_1 'no. entries in DATA_1
  updates=1
  lightcount=0
  eventcount=0
  leddelay=8000000/GLOBALDELAY
  
  ' Make sure the DDS lines are low. 0 resets channel
  DIGOUT_F(2,clock,0)
...
EVENT:
  ...
  IF((99>DATA_1[counts]) and (DATA_1[counts]>=1)) then  'A:Check each element of DATA_1 for an update
    For i=1 to Data_1[counts] 'B: Loop over number of updates at this time
      ...
      '***********************Digital outs***************
      if(ch=101) then
        val_lower=Data_4[updates]
        DIG_WRITELATCH32(2,val_lower) 'Writes Digital Channels 1-32
      endif
  
      if(ch=102) then
        val_upper=Data_4[updates]
        DIG_WRITELATCH32(3,val_upper) 'Writes Digital Channels 101-132
      endif
      
      if(ch=103) then 'for 3rd digital card
        val_upper=Data_4[updates]
        DIG_WRITELATCH32(4,val_upper) 'Writes Digital Channels 133-164
      endif
...
FINISH:
  ...
  'if Data_4[j]=0, i.e. channel should be set to zero, then set to zero
  For i= 1 to 8
    if(Data_5[i]=1) then
      WRITEDAC(1,i,V(0))   
    ENDIF
    if(Data_5[i+8]=1) then
      WRITEDAC(2,i,V(0))   
    ENDIF
    if(Data_5[i+16]=1) then
      WRITEDAC(3,i,V(0))   
    ENDIF
    if(Data_5[i+24]=1) then
      WRITEDAC(4,i,V(0))   
    ENDIF
    if(Data_5[i+32]=1) then
      WRITEDAC(5,i,V(0))   
    ENDIF
    if(Data_5[i+40]=1) then
      WRITEDAC(6,i,V(0))   
    ENDIF
  Next i

  DIG_WRITELATCH32(2,PAR_5) 
  DIG_WRITELATCH32(3,PAR_6)
  DIG_WRITELATCH32(4,PAR_7) 'for 3rd digital card
...
SUB AnalogWrite(achannel, avalue) 
  ...
  if((achannel>=33)and(achannel<=40)) then
    writedac(5,achannel-32,V(avalue))
  endif
  
  if((achannel>=41)and(achannel<=48)) then
    writedac(6,achannel-40,V(avalue))
  endif
...
SUB DDS1Write(digvalue)
  ...
  clock=30        ' data clock
  ioupdate=29      ' update line.  transfers data from buffer into registers
  'ioreset=28      ' resets input buffers - 2020-09-24 commented out, not used
  masterreset=27
...
ENDSUB
'***************************************************
' Commented out 2020-09-24, havne't used DDS2 for a long time, want
' to leave digital channels open for use
' -------------------------------------------
...
'Flashing LEDs -> running cycle/pretty colours
