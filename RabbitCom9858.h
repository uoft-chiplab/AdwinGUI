...

{
	...
	else
	{
		...
		{
		RB9858LibErr("Unacceptable Profile Setting [RCh199]\n");
printf("Error:  Unacceptable Profile Setting [RCh199]\n" );             ///ALAN
		*cmdLength=0;
		}
...
/*{
	...
	if((divisor==1)||(divisor==2)||(divisor==4))
	{
		*cmdLength=SET_PFD_DIVIDER_SIZE;
		...
		///printf("cmd[0] = %x \t SET_PFD_DIVIDER/n",cmd[0]);
		cmd[1]=divisor;
	}
	else
	{
...
printf("Error:  Bad CP Ref Setting [RCh301]\n" );             ///ALAN
		*cmdLength=0;
	}
	return cmd;
}
    ...
    cmd[1]-cmd[4] are DFTW, cmd[5]-cmd[6] are the DFRRW */
{
	...
	if ((fabs(rampHeight)>0.0)&&(fabs(rampHeight)<400.0)&&(rampTime>0.0)&&(rampTime<pow(10,9)))
	{
			...
			*cmdLength=RAMPDDS_START_SIZE;
			...
			{
				RB9858LibErr("Max DDS Ramp rate exceeded\n");
				printf("Error:  Max DDS Ramp rate exceeded\n" );
				*cmdLength=0;
			}
			...
			{
			 	RB9858LibErr("Below Min DDS Ramp rate\n");
 ...
 unsigned char* cmd_begin9854DDSRamp(double rampHeight,double rampTime, double stopFreq, int *cmdLength)
/*	rampHeight in MHz (can be negative),rampTime in ms
    cmd[1]-cmd[6] are DFTW, cmd[7]-cmd[9] are the DFRRW cmd[10]-cmd[15] are stop freq */
{
	...
	if ((fabs(rampHeight)>0.0)&&(fabs(rampHeight)<400.0)&&(rampTime>0.0)&&(rampTime<pow(10,9)))
	{
			...
			*cmdLength=RAMP9854DDS_START_SIZE;
			...
			else if (fabs(dftw_over_dfrrw)>pow(2.0,24.0))
			{
				dfrrw=1-1;
